...
#### Added (Sprint 7 — PR #13 — HITL Gates & Guardrails, 2026-06-21)
- `orchestrator/guardrails.py`: **`GuardrailsEngine`** — single source of truth for run-safety caps (project budget, inner/outer loop iteration limits, per-agent token limits, HITL timeout). Reads `config/guardrails.yaml` (injectable for tests). `orchestrator/edges.py` (`cost_check`/`should_continue_inner_loop`/`should_escalate`) now delegate to it — no more magic numbers in the routers (behavior unchanged).
- `orchestrator/hitl.py`: **`HITLGate(gate_type)`** — two real gates (`security`, `deploy`) built on LangGraph's dynamic `interrupt()`. `request()` persists a **pending row, resumes via `Command(resume=...)`, then records the resolution**; `interpret()` parses dict/str/bool resume values (defaults to **reject** on ambiguous input — a gate never auto-approves). Persistence to the `hitl_approvals` table is **idempotent (find-or-create)** and **degrades gracefully** when the DB is down (like the Docker tools); `expire_pending()`/`is_expired()` implement the timeout. Mirrors the deterministic-gate philosophy (decisions #6/#7).
- `orchestrator/nodes.py`: `hitl_gate` stub → **real security gate** (approve → test generation, reject → back to Coder); new **`deploy_gate`** node after the Reviewer (approve → Deployer, reject → abort/`failed`). `deployer` gains a best-effort **`auto_merge`** hook (only when `repo` + `pr_number` + `GITHUB_TOKEN` present; graceful on failure). HITL request/resolution counters recorded.
- `orchestrator/graph.py`: rewired — `hitl_gate -(hitl_route)-> test_generator | coder`; `reviewer -(review_decision)-> deploy_gate | coder | END`; `deploy_gate -(hitl_route)-> deployer | END`. The two gates pause via `interrupt`, so the graph **requires a checkpointer** (`InMemorySaver` for CLI/tests, `PostgresSaver` for prod). `orchestrator/edges.py`: added `hitl_route()`. `orchestrator/state.py`: added `hitl_decision`/`pr_number`/`repo` channels.
- `api/main.py`: **`POST /api/projects`** (start a run; returns the HITL-paused state with the gate payload) + **`POST /api/hitl/{project_id}/approve`** (resume with approve/reject via `Command(resume=...)`; 404 unknown project, 409 nothing pending). `create_app(checkpointer=...)` defaults to in-memory; prod injects `PostgresSaver`.
- `observability/metrics.py`: added `hitl_requests_total{gate}` + `hitl_resolutions_total{gate,decision}` collectors and `record_hitl_*` helpers.
- `src/__main__.py`: CLI runner now compiles with an in-memory checkpointer and **auto-approves** the HITL gates so `python -m src` still runs end-to-end (the interactive approve/reject flow lives in the API). `config/guardrails.yaml`: added `hitl_timeout_seconds`.
- Tests: `test_guardrails.py` (caps/thresholds/timeout), `test_hitl.py` (interpret/payload/request, idempotent persistence + resolution + timeout against in-memory SQLite ORM, graceful DB-down), plus HITL interrupt/resume flow, deploy/security routing, deployer auto-merge, and API lifecycle cases in `test_orchestrator.py`/`test_api.py`/`test_cli.py`/`test_metrics.py`. **223 passing**, 1 skipped (Postgres); `mypy --strict` clean (49 files); `ruff` clean (new code). **Sprint 7 complete.**
