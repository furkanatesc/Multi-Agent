...
class LiteLLMClient:
    ...
    def completion(
        ...
        **kwargs: Any
        ...
        """
        logger.debug(
            "Requesting LLM completion", model=model, message_count=len(messages)
        )
        ...
        try:
            ...
            if usage:
                ...
                # Export token/cost to Prometheus (defensive: never break a call).
                try:
                    metrics.record_llm_usage(
                        model, prompt_tokens, completion_tokens, cost
                    )
                except Exception:
                    logger.warning("Could not record LLM usage metrics", model=model)

            metrics.record_llm_request(model, "success")
            return response

        except Exception as e:
            metrics.record_llm_request(model, "error")
            logger.error(
                "LLM completion request failed after retries and fallbacks",
                model=model,
                error=str(e),
            )
            raise e
